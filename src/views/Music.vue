    ...
    <el-dialog
      ...
      <div v-loading="audio.waiting" style="text-align:center" @click="closeVol">
        ...
        <div v-if="lrcShow" :class="{rotating:audio.playing}" class="logo" @click="lrcShow = !lrcShow">
		  <el-col :span="20" style="text-align:center" @click="lrcShow = !lrcShow">
		    <span @click="startPlayOrPause">
		  	 <button type="button" class="bg" style="size:18px; border-radius: 50px; background-color: #606266; color:white">stop/start</button>
		      <i v-if="!audio.playing" class="el-icon-ali-play"/>
		      <i v-if="audio.playing" class="el-eicon-ali-pause"/>
		    </span>
		  </el-col>
        </div>
        <!-- 歌词 -->
...
export default {
  ...
  data: function() {
    return {
      ...
      currentTime: 0,
      logoUrl: imglogo,
      songUrl: require("../assets/music/1.mp3"),
      lrcObj:"暂无歌词", // 歌词
      lrc:"暂无歌词", // 歌词
      lrcList:"暂无歌词",
      lrcIndex: 0,
      title: '木偶人',
	  music: require("../assets/music/2.mp3"),
      audio: {
        currentTime: 0,
        maxTime: 0,
        playing: false,
